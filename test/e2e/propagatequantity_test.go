...
var _ = ginkgo.Describe("namespace scope resource propagation quantity testing", func() {
	...
	var startTime, rbTime, workTime, finishTime time.Time
	...
	ginkgo.AfterEach(func() {
		fmt.Printf("rb time: %f\n", rbTime.Sub(startTime).Minutes())
		fmt.Printf("work time: %f\n", workTime.Sub(rbTime).Minutes())
		fmt.Printf("finish time: %f\n", finishTime.Sub(startTime).Minutes())
	...
	ginkgo.It("deployment propagation testing", func() {
		gomega.Eventually(func() bool {
			rbs, err := karmadaClient.WorkV1alpha2().ResourceBindings(testNamespace).List(context.TODO(), metav1.ListOptions{})
			if err != nil {
				return false
			}

			return len(rbs.Items) == quantityNum
		}, pollTimeout, pollInterval).Should(gomega.Equal(true))
		rbTime = time.Now()

		gomega.Eventually(func() bool {
			works, err := karmadaClient.WorkV1alpha1().Works("karmada-es-member1").List(context.TODO(), metav1.ListOptions{})
			if err != nil {
				return false
			}

			return len(works.Items) > quantityNum
		}, pollTimeout, pollInterval).Should(gomega.Equal(true))
		workTime = time.Now()
