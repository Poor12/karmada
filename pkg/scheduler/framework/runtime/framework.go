...
func defaultFrameworkOptions() frameworkOptions {
	return frameworkOptions{
		...
		parallelizer:    lifted.NewParallelizer(lifted.DefaultParallelism),
	}
}

// Parallelizer returns a parallelizer holding parallelism for scheduler.
func (frw *frameworkImpl) Parallelizer() lifted.Parallelizer {
...
func WithParallelism(parallelism int) Option {
	return func(o *frameworkOptions) {
		o.parallelizer = lifted.NewParallelizer(parallelism)
	}
}
...
func (frw *frameworkImpl) RunFilterPlugins(ctx context.Context, placement *policyv1alpha1.Placement, resource *workv1alpha2.ObjectReference, cluster *clusterv1alpha1.Cluster) (result *framework.Result) {
	startTime := time.Now()
	defer func() {
	...
	for _, p := range frw.filterPlugins {
		if result := frw.runFilterPlugin(ctx, p, placement, resource, cluster); !result.IsSuccess() {
			return result
		}
	}
...
//TODO: metrics need to be configurable
func (frw *frameworkImpl) runFilterPlugin(ctx context.Context, pl framework.FilterPlugin, placement *policyv1alpha1.Placement, resource *workv1alpha2.ObjectReference, cluster *clusterv1alpha1.Cluster) *framework.Result {
	startTime := time.Now()
	result := pl.Filter(ctx, placement, resource, cluster)
...
func (frw *frameworkImpl) RunScorePlugins(ctx context.Context, placement *policyv1alpha1.Placement, spec *workv1alpha2.ResourceBindingSpec, clusters []*clusterv1alpha1.Cluster) (ps framework.PluginToClusterScores, result *framework.Result) {
	...
	for _, p := range frw.scorePlugins {
		pluginToClusterScores[p.Name()] = make(framework.ClusterScoreList, len(clusters))
	}
	ctx, cancel := context.WithCancel(ctx)
	errCh := lifted.NewErrorChannel()
	...
	frw.Parallelizer().Until(ctx, len(frw.scorePlugins), func(index int) {
		...
		for i, clusterScore := range clusterScoreList {
			...
			clusterScoreList[i].Score = clusterScoreList[i].Score * int64(weight)
		}
	})
	if err := errCh.ReceiveError(); err != nil {
