...
func (c *ReplicaCalculator) GetResourceReplicas(ctx context.Context, currentReplicas int32, targetUtilization int32, resource corev1.ResourceName, namespace string, selector labels.Selector, container string, podList []*corev1.Pod, calibration int32) (replicaCount int32, utilization int32, rawUtilization int64, timestamp time.Time, err error) {
	...
	if !scaleUpWithUnready && len(missingPods) == 0 {
		...
		return int32(math.Ceil(usageRatio * float64(readyPodCount) / float64(calibration))), utilization, rawUtilization, timestamp, nil
	}
	...
	newReplicas := int32(math.Ceil(newUsageRatio * float64(len(metrics)) / float64(calibration)))
...
func (c *ReplicaCalculator) GetRawResourceReplicas(ctx context.Context, currentReplicas int32, targetUsage int64, resource corev1.ResourceName, namespace string, selector labels.Selector, container string, podList []*corev1.Pod, calibration int32) (replicaCount int32, usage int64, timestamp time.Time, err error) {
	metrics, timestamp, err := c.metricsClient.GetResourceMetric(ctx, resource, namespace, selector, container)
	if err != nil {
...
func (c *ReplicaCalculator) calcPlainMetricReplicas(metrics metricsclient.PodMetricsInfo, currentReplicas int32, targetUsage int64, resource corev1.ResourceName, podList []*corev1.Pod, calibration int32) (replicaCount int32, usage int64, err error) {
	if len(podList) == 0 {
	...
	if !scaleUpWithUnready && len(missingPods) == 0 {
		...
		// if we don't have any unready or missing pods, we can calculate the new replica count now
		return int32(math.Ceil(usageRatio * float64(readyPodCount) / float64(calibration))), usage, nil
	}
