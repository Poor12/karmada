...
| Status  | SLI                                           | SLO                                                                                                |
| --------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| Offical | 最近5min对单个资源对象Mutating API调用（包括POST、PUT、DELETE、PATCH）的P99时延    | 除聚合API和CRD外, P99 <= 1s                                                                        |
| Offical | 最近5min的non-streaming的只读API调用（包括GET和LIST）的P99时延 | 除聚合API和CRD外, Scope=resource, P99 <= 1s, Scope=namespace, P99 <= 5s, Scope=cluster, P99 <= 30s |
...
| Number of Pods per Namespace | 100000        |
...
| metric   | P50(ms) | P90(ms) | P99(ms) | SLO   |
| ---------- | --------- | --------- | --------- | ------- |
| cluster_schedule | xxxxxx  | xxxxx   | xxxxx   | xxxxx |
| pod_startup | xxxxxx  | xxxxx   | xxxxx   | xxxxx |
| pod_startup_with_status | xxxxxx  | xxxxx   | xxxxx   | xxxxx |
